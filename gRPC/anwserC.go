...
var commands = map[string]struct {
    ...
    "multiConcurrentCall": {
		name:  "multiConcurrentCall",
		desc:  "multiConcurrentCall the provided book",
		do:    multiConcurrentCall,
		usage: "client.go multiConcurrentCall <num_of_concurrent>",
	},
	
}

func multiConcurrentCall(ctx context.Context, args ...string) {
    start := time.Now()
    num_of_concurrent, err := strconv.ParseInt(args[0], 10, 64)
    if err != nil {
		log.Fatalf("printResp (%v): %v", num_of_concurrent, err)
	}
	book := &pb.Book{
		Id:     1,
		Title:  "drive",
    ...
    defer conn.Close()
    for i := 0; i < int(num_of_concurrent); i++ {
		if i%4 == 0{
			go client.Insert(ctx, book)
		}else if i%4 == 1{
			go client.Get(ctx, &pb.BookIdRequest{int32(1)})
		}else if i%4 == 2{
			go client.List(ctx, &pb.Empty{})
		}else{
			go client.Delete(ctx, &pb.BookIdRequest{int32(1)})
		}
	}
